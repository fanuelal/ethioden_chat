...
const RoomModel = class {
  ...
  static getRoomsByUserId = async (userId, type) => {
    ...
    WHERE
      (m.memberId = '${userId}' OR r.created_by = '${userId}')
      AND r.type = '${type}'
...
`;
  ...
  static getAll = async (type, userId) => {
    try {
      const fetchedData = await new Promise((resolve, reject) => {
        con.query(
          `SELECT * FROM rooms WHERE isDeleted = false AND type = '${type}';`,
          (err, result, fields) => {
            if (err) reject(err);
            resolve(result);
          }
  ...
  static getSingle = async (roomId) => {
    return new Promise((resolve, reject) => {
      con.query(
        `SELECT DISTINCT r.name, r.created_by, e.id, e.first_name, e.last_name, e.phone_num, e.email, e.department, e.isActive, e.isDeleted, e.role FROM employees AS e INNER JOIN members AS m ON m.memberId = e.id INNER JOIN rooms AS r ON m.roomId = r.id WHERE m.roomId = '${roomId}' AND m.isLeaved = false;
        `,
        (err, result, fields) => {
          if (err) reject(err);
          resolve(result);
  ...
  updateRoom = async (roomId) => {
    return new Promise((resolve, reject) => {
      con.query(
        `UPDATE rooms SET  name = '${this.name}', type = '${
          this.type
        }', members = '${JSON(this.members)}', WHERE id = '${roomId}'`,
        (error, result, fields) => {
          if (error) reject(error);
          resolve(result);
        }
  ...
  static deleteRoom = async (roomId) => {
    ...
    return new Promise((resolve, reject) => {
      con.query(
        `UPDATE rooms SET  isDeleted = true  WHERE id = '${roomId}'`,
        (error, result, fields) => {
          if (error) reject(error);
          resolve(result);
        }
